  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pt-4 pb-24">
      ...
      <input
        ...
        className="w-full bg-[var(--sc-surface)] border border-[var(--sc-border)] rounded-lg px-3 py-2 text-sm mb-4 outline-none"
      />
      {teams.length > 0 ? (
        <>
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))' }}>
            {teams.map(t => (
              <Link key={t.id} href={`/teams/${t.id}`} className="bg-[var(--sc-surface)] border border-[var(--sc-border)] rounded-lg p-2 text-center no-underline text-inherit hover:shadow-md transition-shadow">
                {t.logoUrl && <img src={t.logoUrl} alt="" className="w-8 h-8 mx-auto mb-1" />}
          ...
          {hasMore && (
            <div className="flex justify-center mt-4">
              <button onClick={() => load(search, teams.length, false)} disabled={loading} className="px-4 py-2 text-sm font-semibold border border-[var(--sc-border)] rounded-lg bg-[var(--sc-surface)] cursor-pointer disabled:opacity-50">
        ...
        <p className="text-sm text-center text-[var(--sc-text-secondary)]">{search ? 'No teams found' : 'Search for a team'}</p>
      ) : null}
    </div>
  );
}
