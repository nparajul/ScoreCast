    ...
    <div className="py-4 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs opacity-50">{data.standings.length} member{data.standings.length !== 1 ? "s" : ""}{data.startingGameweekNumber && data.startingGameweekNumber > 1 ? ` · Since GW${data.startingGameweekNumber}` : ""}</p>
        </div>
        <Link href={`/predict/${data.seasonId}`} className="px-4 py-1.5 rounded-full text-xs font-bold text-white" style={{ background: "var(--sc-tertiary)" }}>
          Make Predictions
        </Link>
      </div>
      ...
      <div className="rounded-xl border border-white/10 overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-[10px] opacity-50">
              ...
              <th className="px-2 py-1.5 text-center">GW</th>
              <th className="px-2 py-1.5 text-center">Exact</th>
                ...
                <tr key={row.userId} className="border-t border-white/5">
                  ...
                  <td className="px-1 py-2">
                    <Link href={href} className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-xs overflow-hidden shrink-0">
                        {row.avatarUrl ? <img src={row.avatarUrl} alt="" className="w-full h-full object-cover" /> : "👤"}
                      </div>
                      <span className="font-semibold truncate max-w-[120px] underline underline-offset-2 decoration-white/20">{row.displayName}</span>
                    </Link>
                  </td>
