    ...
    ILogger<SyncPulseEventsCommandHandler> Logger) : ICommandHandler<SyncPulseEventsCommand, ScoreCastResponse<SyncPulseEventsResult>>
{
    ...
    private async Task<ScoreCastResponse<SyncPulseEventsResult>> ExecuteCoreAsync(SyncPulseEventsCommand command, CancellationToken ct)
    {
        ...
        // Get finished match IDs already processed (have events OR marked as Pulse-synced)
        var matchIdsWithEvents = await DbContext.MatchEvents
            .Where(e => e.Match.Gameweek.SeasonId == currentSeason.Id)
            .Select(e => e.MatchId)
            .Distinct()
            .ToListAsync(ct);
        var pulseSyncedMatchIds = await DbContext.ExternalMappings
            ...
            .Select(m => m.EntityId)
            .ToListAsync(ct);
        var processedMatchIds = matchIdsWithEvents.Concat(pulseSyncedMatchIds).ToHashSet();
