    ...
    IScoreCastDbContext DbContext) : ICommandHandler<GenerateInsightsCommand, ScoreCastResponse>
{
    public async Task<ScoreCastResponse> ExecuteAsync(GenerateInsightsCommand command, CancellationToken ct)
    {
        ...
        foreach (var comp in comps.Data)
        {
            ...
            var insightsRequest = new GetMatchInsightsRequest
            {
                SeasonId = current.Id,
                GameweekNumber = gw.Data.CurrentGameweek,
                AppName = command.Request.AppName
            };

            var result = await new GetOrCreateMatchInsightsCommand(insightsRequest).ExecuteAsync(ct);
            if (result is { Success: true, Data.Count: > 0 }) generated += result.Data.Count;

            if (gw.Data.CurrentGameweek < gw.Data.TotalGameweeks)
            {
                insightsRequest = new GetMatchInsightsRequest
                {
                    SeasonId = current.Id,
                    GameweekNumber = gw.Data.CurrentGameweek + 1,
                    AppName = command.Request.AppName
                };

                var next = await new GetOrCreateMatchInsightsCommand(insightsRequest).ExecuteAsync(ct);
